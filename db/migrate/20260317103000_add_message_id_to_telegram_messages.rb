# frozen_string_literal: true

class AddMessageIdToTelegramMessages < ActiveRecord::Migration[8.0]
  CHANNEL_TDLIB_MESSAGE_ID_BASE = 300_000_000_000
  CHANNEL_TDLIB_MESSAGE_ID_MAX = 400_000_000_000
  TDLIB_MESSAGE_ID_SHIFT = 20

  def up
    add_column :telegram_messages, :message_id, :bigint
    add_index :telegram_messages, [ :td_chat_id, :message_id ]

    execute <<~SQL.squish
      UPDATE telegram_messages
      SET message_id = CASE
        WHEN td_message_id >= #{CHANNEL_TDLIB_MESSAGE_ID_BASE}
          AND td_message_id < #{CHANNEL_TDLIB_MESSAGE_ID_MAX}
          THEN td_message_id - #{CHANNEL_TDLIB_MESSAGE_ID_BASE}
        WHEN td_message_id > 0
          THEN (td_message_id >> #{TDLIB_MESSAGE_ID_SHIFT})
        ELSE NULL
      END
    SQL
  end

  def down
    remove_index :telegram_messages, column: [ :td_chat_id, :message_id ]
    remove_column :telegram_messages, :message_id
  end
end
